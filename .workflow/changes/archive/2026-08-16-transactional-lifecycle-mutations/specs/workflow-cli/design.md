...
`workflow-cli` owns local lifecycle state calculation, schema-driven artifact validation, semantic delta preparation, capability synchronization, archive movement, lifecycle mutation transactions, and read-only local diagnostics.
...
`schema-pack` declares artifact roles and publication paths. `deploy-kit` builds and validates the published CLI and consumes CLI Doctor results rather than reimplementing selected-schema, main-spec, or transaction validation. `workflow-runtime` routes agent intent to exact local commands and contracts.
