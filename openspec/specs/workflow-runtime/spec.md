...
Expose one contract-first OpenSpec lifecycle consistently through Cursor and Codex.
...
Cursor SHALL receive `/opsx:*` commands and one always-applied router. Codex SHALL receive a concise skill, AGENTS managed routing, and generated references. Neither adapter SHALL be authoritative over the other. Routing SHALL cover explicit OpenSpec lifecycle intent and failures within an active lifecycle operation; it SHALL NOT capture unrelated bugs or test failures.

#### Scenario: Natural-language lifecycle request
- **WHEN** a request clearly maps to an OpenSpec lifecycle operation
