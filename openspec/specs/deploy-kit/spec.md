...
Init SHALL copy the workflow-owned `.workflow/pack`, preserve project-owned `.workflow/mcp.json`, `.workflow/rules.json`, and `.workflow/rules/`, then generate Cursor and Codex runtime assets. Client adapter files SHALL NOT be used as source input.
